...
export default function DepartamentoPage() {
  ...
  const [sinActividadRegistros, setSinActividadRegistros] = useState<
    SinActividadRegistro[]
  >([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setPostsNacionales(
          (data.postsNacionales || []).map((p: any) => ({
            ...p,
            fechapublicacion: p.fechapublicacion || "",
          }))
        );
...
const departamentos = [
  ...
  const allPostsGeneral: PostGeneral[] = allPosts;
  const postsNacionalesGeneral: PostGeneral[] = postsNacionales;

  const postsPorTitularidad: Record<string, PostGeneral[]> = Object.fromEntries(
    titularidades.map((tit) => {
      let posts = allPostsGeneral.filter(
      ...
      if (tit === "PRESIDENTE" || tit === "VICEPRESIDENTE") {
        ...
        if (nacionales.length > 0) {
          ...
          posts = [
            ...nacionales,
            ...posts.filter((p) => !idsNacionales.has(p.id)),
          ];
        }
      }
      return [tit, posts];
          ...
          <div className="flex items-center gap-2 w-full">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <BreadcrumbLink href={`/dashboard/${departamento}`}>
                    {departamentoNombre}
                  </BreadcrumbLink>
                </BreadcrumbItem>
              </BreadcrumbList>
      ...
      <div className="w-full max-w-5xl mx-auto px-2 sm:px-6 py-2 sm:py-4 text-sm">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2 gap-4">
          <h1 className="text-2xl sm:text-2xl font-bold">
            Publicaciones por Titularidad - {departamentoNombre}
          </h1>
        </div>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4 w-full">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full sm:w-auto flex-1">
            ...
            <input
              ...
              max={hasta}
            />
            <label htmlFor="hasta" className="font-medium ml-4">
              Hasta:
            </label>
            <input
              id="hasta"
        ...
        <div className="flex flex-col gap-12">
          {titularidades
            .filter((tit) => tit !== "SIN ACTIVIDAD EN RRSS")
            .map((tit: string) => (
              <section key={tit}>
                ...
                {postsPorTitularidad[tit].length === 0 ? (
                  <div className="flex flex-col items-center justify-center min-h-[30px]">
                    <h3 className="text-lg text-muted-foreground">
                      No hay publicaciones extraídas para esta titularidad
                    </h3>
                  </div>
                ) : (
                      ...
                      <Table className="w-full min-w-[500px] sm:min-w-[900px] border border-gray-200 rounded-lg bg-white text-xs">
                        <TableHeader>
                          <TableRow className="bg-gray-100">
                            <TableHead className="px-1 py-1 min-w-[30px]">
                              Nombre
                            </TableHead>
                            <TableHead className="px-1 py-1 min-w-[120px]">
                              Texto
                            </TableHead>
                            <TableHead className="px-1 py-1 w-16 text-center">
                              Me gusta
                            </TableHead>
                            <TableHead className="px-1 py-1 w-16 text-center hidden md:table-cell">
                              Comentarios
                            </TableHead>
                            <TableHead className="px-1 py-1 w-16 text-center hidden md:table-cell">
                              Compartidos
                            </TableHead>
                            <TableHead className="px-1 py-1 w-20 hidden lg:table-cell">
                              Red Social
                            </TableHead>
                            <TableHead className="px-1 py-1 w-24 hidden lg:table-cell">
                              Fecha y hora
                            </TableHead>
                            <TableHead className="px-1 py-1 w-14 text-center hidden lg:table-cell">
                              Ver post
                            </TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {postsPorTitularidad[tit].map((post: PostGeneral) => (
                            ...
                            >
                              ...
                              <TableCell
                                className="px-1 py-1 max-w-[120px] truncate"
                                title={post.texto}
                              >
                                {post.texto?.slice(0, 50)}
                                {post.texto?.length > 50 ? "..." : ""}
                              </TableCell>
                              <TableCell className="px-1 py-1 text-center">
                                {post.likes}
                              </TableCell>
                              <TableCell className="px-1 py-1 text-center hidden md:table-cell">
                                {post.comentarios}
                              </TableCell>
                              <TableCell className="px-1 py-1 text-center hidden md:table-cell">
                                {post.compartidos}
          ...
          <section key="sin-actividad" className="mb-12">
            ...
            {sinActividadRegistros.length === 0 ? (
              <div className="flex flex-col items-center justify-center min-h-[100px]">
                <h3 className="text-lg text-muted-foreground">
                  Todos tuvieron actividad en las últimas 24 horas
                </h3>
              </div>
            ) : (
              <div className="w-full overflow-x-auto p-2">
